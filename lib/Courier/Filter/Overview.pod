...
=item B<The Courier MTA's courierfilter interface>

=item B<About Courier::Filter>

=item B<Courier::Filter compared to other courierfilter implementations>

=item B<Using Courier::Filter>

=item B<How Courier::Filter modules work>

=item B<A sample HeaderSimple filter module>

=item B<Testing Courier::Filter modules>

=item B<Bundled Courier::Filter modules>

=item B<Bundled Courier::Filter loggers>
...
Python, this is clearly your choice.  As of version 0.5, there is some control
file parsing, but no message parsing infrastructure.
...
include path (see the last lines of `perl -V`).  You can either use prepared
modules (see L</"Bundled Courier::Filter modules"> for a list of modules that
come with Courier::Filter), or you can write your own (see L</"How
Courier::Filter modules work">).
...
required.  For example, this is how a simple configuration file could look
like:
...
symlink pointing to C<pureperlfilter> (which is used for bootstrapping
Courier::Filter) in C</etc/courier/filters/active/>:
...
=head2 How Courier::Filter modules work
...
L</"Writing Courier::Filter modules"> if you want to know why.)
...
=head2 Writing Courier::Filter modules
...
result> as described in L</"How Courier::Filter modules work">.
...
we will create a simple variant of the C<Header> module that exactly matches a
...
Literal and reg-ex matching of one or more message header fields

=item I<MIMEParts>

Size and MD5 sum matching of message MIME parts

=begin comment

=item I<SPF>

Senders Permitted From authorization checking of the calling MTA's IP address
against the envelope sender domain
...
Courier::Filter is Copyright (C) 2003-2004 Julian Mehnle <julian@mehnle.net>.
All rights reserved.
